...
export default function GroupDetailPage() {
    ...
    // --- MODIFIED: Implements group leaving logic with confirmations ---
    const handleLeaveGroup = async () => {
        const isOwner = userRole === "owner";
        let confirmationMessage = "Are you sure you want to leave this group?";
        
        // Calculate members excluding the current user to predict the outcome
        const currentUserId = getUserIdFromToken();
        ...
        if (isOwner) {
            if (remainingMembersCount === 0) {
                ...
                // Find the first non-admin member to mention for ownership transfer
                const nextAdminCandidate = otherMembers
                    .find(m => m.role?.toLowerCase() !== 'admin' && m.userId !== group.createdBy?.userId);

                if (nextAdminCandidate) {
                    confirmationMessage = 
                        `⚠️ WARNING: You are the Group Admin/Owner. Leaving will transfer ownership to **${nextAdminCandidate.name || 'another member'}**. Are you sure you want to proceed?`;
                } else {
                    // Fallback, likely a complex admin scenario or all other members are already admins
                    confirmationMessage = "⚠️ WARNING: You are the Group Admin/Owner. Leaving will result in ownership transfer. Are you sure you want to proceed?";
                }
            }
        }
        ...
        try {
            const response = await fetch(`http://localhost:8145/api/groups/leave/${groupId}`, {
                method: "DELETE",
                headers: { Authorization: `Bearer ${token}` },
            });
            ...
            if (!response.ok) {
                const errorMessage = data.message || `Failed to leave group (Status: ${response.status})`;
                alert(`Error: ${errorMessage}`);
                return;
            }

            // Success: Show message and redirect to the groups list
            alert(`Success: ${data.message}`);
            navigate("/my-groups"); // Redirect after successfully leaving/deleting the group

...
const ValidationItem = ({ isValid, text }) => (
    <li className={`flex items-center text-sm ${isValid ? 'text-green-600' : 'text-red-500'} transition-colors duration-300`}>
        {isValid ? (
...
const GroupCreateForm = ({ courses = [], onSubmit, onCancel }) => {
  ...
  // State for live passkey validation
  ...
  // Handle passkey input changes and perform live validation
  const handlePasskeyChange = (e) => {
    const newPasskey = e.target.value;
    setPasskey(newPasskey);

    setPasskeyValidation({
      minLength: newPasskey.length >= 4,
      ...
      <div className="max-w-3xl w-full mx-auto p-8 bg-white rounded-2xl shadow-xl space-y-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {privacy === 'private' && (
                 <div>
                    <label htmlFor="passkey" className="block text-sm font-semibold text-gray-700 mb-1">
                    ...
                    <div className="relative">
                        ...
                        >
                            {showPasskey ? (
                                ...
                                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M3.707 2.293a1 1 0 00-1.414 1.414l14 14a1 1 0 001.414-1.414l-1.473-1.473A10.014 10.014 0 0019.542 10C18.27 7.548 15.245 4.5 10 4.5c-1.536 0-2.97.408-4.252 1.154l-1.04-1.04A1 1 0 003.707 2.293zM10 12a2 2 0 100-4 2 2 0 000 4z" clipRule="evenodd" /><path d="M10 15c-3.267 0-6.08-2.5-7.35-5.632A10.003 10.003 0 014.39 6.353l1.181 1.181A4.01 4.01 0 0010 14a4 4 0 003.44-1.896l1.362 1.362A9.95 9.95 0 0110 15z" /></svg>
                            )}
                        </button>
                    </div>
                 </div>
            ...
            {privacy === 'private' && passkey && (
                <div className="mt-3 p-4 bg-gray-50 rounded-lg border border-gray-200">
                    <p className="text-sm font-semibold text-gray-600 mb-2">Your passkey must contain:</p>
                    <ul className="space-y-1.5">
                        <ValidationItem isValid={passkeyValidation.minLength} text="At least 4 characters" />
                        <ValidationItem isValid={passkeyValidation.hasLower} text="A lowercase letter (a-z)" />
                        <ValidationItem isValid={passkeyValidation.hasUpper} text="An uppercase letter (A-Z)" />
                        <ValidationItem isValid={passkeyValidation.hasNumber} text="A number (0-9)" />
                        <ValidationItem isValid={passkeyValidation.hasSpecial} text="A special character (!@#$%^&*)" />
                    </ul>
                </div>
            )}
